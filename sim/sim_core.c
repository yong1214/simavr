...
// Make available for diagnostics in other translation units
uint16_t
_avr_flash_read16le(
	avr_t * avr,
...
void avr_core_watch_write(avr_t *avr, uint16_t addr, uint8_t v)
{
	...
	// DEBUG: Log critical register writes to verify storage
	// if (addr == 0xC0 || addr == 0xC1 || addr == 0xC6 || addr == 0x44 || addr == 0x45 || addr == 0x24 || addr == 0x25) {
	//	fprintf(stderr, "🔍 CORE WRITE: addr=0x%02X value=0x%02X -> avr->data[0x%02X]=0x%02X\n", 
