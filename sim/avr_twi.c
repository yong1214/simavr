...
//	AVR_TRACE(avr, "cleared %d\n", cleared);
...
#endif
		// DISABLED: I2C logging (too verbose, can be re-enabled for debugging)
		// Log I2C STOP for backend monitoring
		// fprintf(stderr, "I2C_STOP\n");
		// fflush(stderr);
...
#endif
		// START logging removed - we log complete transactions at ADDR time
	...
	if (p->state & TWI_COND_SLAVE) {
		...
		if (p->state & TWI_COND_ADDR) {
			...
			// DISABLED: I2C logging (too verbose, can be re-enabled for debugging)
			// Enhanced I2C debugging for backend monitoring
			// Print complete transaction info in ONE line to avoid buffering issues
			// uint8_t addr_7bit = p->peer_addr >> 1;
			// const char* direction = (p->peer_addr & 1) ? "R" : "W";
			// fprintf(stderr, "I2C_TRANSACTION:ADDR:0x%02X:%s\n", addr_7bit, direction);
			// fflush(stderr);

			// we send an IRQ and we /expect/ a slave to reply
			// immediately via an IRQ tp set the COND_ACK bit
			// otherwise it's assumed it's been nacked...
			// With official C-based virtual devices (i2c_eeprom), the ACK
			// comes back synchronously via IRQ callback!
			avr_raise_irq(p->io.irq + TWI_IRQ_OUTPUT,
		...
		void * param)
{
	...
	// DISABLED: I2C logging (too verbose, can be re-enabled for debugging)
	// Enhanced I2C debugging for backend monitoring
	// Log ALL data writes (not just when ADDR bit is set)
	// fprintf(stderr, "I2C_DATA:0x%02X\n", v);
	// fflush(stderr);
	
	// With official C-based virtual devices (i2c_eeprom), the ACK
	// comes back synchronously via IRQ callback when data is processed
	// No need for stdin wait loop!
