		...
		void * param)
{
	...
	// DISABLED: Too verbose, floods output
	// DEBUG: Log timer register writes from Arduino framework
	// printf("🔧 TIMER WRITE: %s-%c addr=0x%02X value=0x%02X (Arduino framework configuring timer)\n", 
	...
	// DEBUG: Timer register write logging (disabled for production)
	// if (addr == 0x44 || addr == 0x45) {
	// 	printf("🔍 TIMER WRITE: addr=0x%02X value=0x%02X -> avr->data[0x%02X]=0x%02X\n", 
	// 		   addr, v, addr, avr->data[addr]);
	// }
