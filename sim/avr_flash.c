...
avr_flash_init(
		...
		avr_flash_t * p)
{
	printf("🔧 avr_flash_init: ENTER\n"); fflush(stdout);
	p->io = _io;
	// printf("%s init SPM %04x BLB %d SIGRD %d\n",
	//		__FUNCTION__, p->r_spm, p->blbset.bit, p->sigrd.bit);

	printf("🔧 avr_flash_init: About to allocate tmppage, size=%d\n", p->spm_pagesize); fflush(stdout);
	if (!p->tmppage)
		p->tmppage = malloc(p->spm_pagesize);
	printf("🔧 avr_flash_init: tmppage allocated at %p\n", p->tmppage); fflush(stdout);

	printf("🔧 avr_flash_init: About to allocate tmppage_used, size=%d\n", p->spm_pagesize / 2); fflush(stdout);
	if (!p->tmppage_used)
		p->tmppage_used = malloc(p->spm_pagesize / 2);
	printf("🔧 avr_flash_init: tmppage_used allocated at %p\n", p->tmppage_used); fflush(stdout);

	printf("🔧 avr_flash_init: About to register IO\n"); fflush(stdout);
	avr_register_io(avr, &p->io);
	printf("🔧 avr_flash_init: About to register vector\n"); fflush(stdout);
	avr_register_vector(avr, &p->flash);

	printf("🔧 avr_flash_init: About to register IO write\n"); fflush(stdout);
	avr_register_io_write(avr, p->r_spm, avr_flash_write, p);
	printf("🔧 avr_flash_init: EXIT\n"); fflush(stdout);
}
