		...
		avr_t * avr)
{
	...
	if (!avr_regbit_get(avr, vector->enable) || !vector->pending) {
		...
		// Safety guard: if the running stack is full, do NOT dispatch
		// the interrupt. Dispatching without tracking on the running stack
		// would desynchronize the stack (RETI pops wrong entry) and can
		// cascade into firmware corruption. Instead, drop the interrupt
		// and clear it to prevent re-queuing.
		if (table->running_ptr == ARRAY_SIZE(table->running)) {
			static int overflow_count = 0;
			if (overflow_count < 5) {
				AVR_LOG(avr, LOG_ERROR, "%s run out of nested stack! vector=%d (dropping)\n", __func__, vector->vector);
				overflow_count++;
				if (overflow_count == 5)
					AVR_LOG(avr, LOG_ERROR, "%s further overflow messages suppressed\n", __func__);
			}
			// Drop the interrupt - don't dispatch, don't jump to ISR
			avr_clear_interrupt(avr, vector);
			avr->interrupt_state = avr_has_pending_interrupts(avr);
			return;
		}
