...
{
	printf("🔧 do_chunk: chunk->addr=0x%04X, chunk->size=%d, chunk->fill_size=%d, chunk->type=%d\n", 
	       chunk->addr, chunk->size, chunk->fill_size, chunk->type);
	fflush(stdout);
		...
		elf_firmware_t * firmware)
{
	...
	printf("🔧 avr_load_firmware: About to load chunks, firmware->chunks=%p\n", (void*)firmware->chunks);
	fflush(stdout);
	for (chunk = firmware->chunks; chunk; ) {
		printf("🔧 avr_load_firmware: Processing chunk=%p, addr=0x%04X, size=%d, type=%d\n", 
		       (void*)chunk, chunk->addr, chunk->size, chunk->type);
		fflush(stdout);
