		...
		void * param)
{
	...
	// DISABLED: Too verbose, floods stderr buffer and hides thread creation messages
	// fprintf(stderr, "GPIO WRITE: PORT%c addr=0x%02X value=0x%02X\n", 
	// 	   p->name, addr, v);

	D(if (avr->data[addr] != v) printf("** PORT%c(%02x) = %02x\r\n", p->name, addr, v);)
	avr_core_watch_write(avr, addr, v);
	
	// DISABLED: Verbose GPIO debug logging
	// if (addr == 0x24 || addr == 0x25) {
	// 	fprintf(stderr, "GPIO WRITE: PORT%c addr=0x%02X value=0x%02X -> avr->data[0x%02X]=0x%02X\n", 
	// 		   p->name, addr, v, addr, avr->data[addr]);
	// }
		...
		void * param)
{
	...
	// DISABLED: Verbose DDR logging (uncomment for debugging)
	// fprintf(stderr, "GPIO DDR WRITE: DDR%c addr=0x%02X value=0x%02X\n", 
	// 	   p->name, addr, v);
...
// Internal wrapper for avr_ioport_read to match avr_io_read_t signature
static uint8_t _avr_ioport_io_read_wrapper(struct avr_t * avr, avr_io_addr_t addr, void * param) {
    avr_ioport_t * p = (avr_ioport_t *)param;
    return avr_ioport_read(avr, addr, p);
}

// Internal wrapper for avr_ioport_pin_write to match avr_io_write_t signature
static void _avr_ioport_io_pin_write_wrapper(struct avr_t * avr, avr_io_addr_t addr, uint8_t v, void * param) {
    avr_ioport_t * p = (avr_ioport_t *)param;
    avr_ioport_pin_write(avr, addr, v, p);
}

//  wrapper for avr_ioport_read
 uint8_t em_avr_ioport_read(avr_t * avr, char port_name) {
    // Access IOPORT directly from MCU structure instead of through I/O system
    struct mcu_t * mcu = (struct mcu_t *)avr;
    avr_ioport_t * p = NULL;

    // Map port name to MCU structure field
    switch (port_name) {
        case 'B':
        case 'b':
            p = &mcu->portb;
        ...
        case 'd':
            ...
            break;
        default:
            AVR_LOG(avr, LOG_ERROR, "IOPORT%c: Unknown port name\n", port_name);
            return 0;
    }

    if (!p) {
        AVR_LOG(avr, LOG_ERROR, "IOPORT%c: Could not find IOPORT instance for read (MCU access failed)\n", port_name);
        return 0;
    }

    // Call the original avr_ioport_read with appropriate dummy values for addr and param
    return avr_ioport_read(avr, p->r_pin, p);
}

//  wrapper for avr_ioport_pin_write
 void em_avr_ioport_pin_write(avr_t * avr, char port_name, uint8_t pin_num, uint8_t level) {
    // Access IOPORT directly from MCU structure instead of through I/O system
    struct mcu_t * mcu = (struct mcu_t *)avr;
    avr_ioport_t * p = NULL;
    ...
    if (!p) {
        AVR_LOG(avr, LOG_ERROR, "IOPORT%c: Could not find IOPORT instance for pin write (MCU access failed)\n", port_name);
        return;
    }

    // Call the original avr_ioport_pin_write with appropriate dummy values for addr and param
    // This is a simplification; it might need to construct the addr/mask based on pin_num
    avr_ioport_pin_write(avr, p->r_pin, (level & 1) << pin_num, p); // Construct v based on level and pin_num
}
...
//		p->name, p->r_port);
	...
	avr_register_io_read(avr, p->r_pin, _avr_ioport_io_read_wrapper, p); // Use wrapper
	avr_register_io_write(avr, p->r_pin, _avr_ioport_io_pin_write_wrapper, p); // Use wrapper
