...
// Forward declaration for DMA thread
struct uart_dma_thread_t;
...
typedef struct avr_uart_t {
	...
	// DMA thread for autonomous UART transmission
	struct uart_dma_thread_t *dma_thread;
} avr_uart_t;
...
// Add these declarations for Emscripten export
// Native build - export functions for Node.js
...
// Native build - export functions for Node.js
const char* em_get_next_uart_string(void);
const char* em_get_all_uart_output(void);
void em_clear_uart_buffer(void);
int em_get_uart_buffer_stats(void);
